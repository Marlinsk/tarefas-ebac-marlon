...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(MemeNaoEncontradoException.class)
    public ResponseEntity<ErrorResponse> handleMemeNaoEncontrado(MemeNaoEncontradoException ex) {
        logger.error("Meme não encontrado: {}", ex.getMessage());
    ...
    public ResponseEntity<ErrorResponse> handleCategoriaInvalida(CategoriaInvalidaException ex) {
        logger.error("Categoria inválida: {}", ex.getMessage());
        ErrorResponse error = new ErrorResponse(HttpStatus.BAD_REQUEST.value(), ex.getMessage(), LocalDateTime.now());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
    }
